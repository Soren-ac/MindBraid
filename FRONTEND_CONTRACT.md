...
| Selectable styles | Colorful (default), Pencil Sketch, Organic Classic, Swiss Editorial, Atlas Cards, Technical Draft, and Charcoal with visual sidebar previews | Independent geometry/effect specs in an injectable style registry |
| Selectable color schemes | Graphite, Spectrum (default), Morandi Mint, Retro Autumn, Coastal Ink, Deep Lagoon, and Coral Tide with visual sidebar previews | Independent semantic/role/branch color specs and user-facing naming in an injectable palette registry; every style/palette cross-pair is valid |
